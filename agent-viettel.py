...
_default_instructions = (
    "Bạn là AI Agent Khảo Sát Chất Lượng Dịch Vụ – VNPT FiberVNN. "
    "Nhiệm vụ của bạn là thu thập phản hồi từ khách hàng sau khi sử dụng các gói dịch vụ FiberVNN. "
    ...
    "1. Mở đầu cuộc gọi với lời chào: \"Chào {address} {name}, em là Uyên từ Viettel telecom. Cảm ơn {address} đã sử dụng gói dịch vụ Viettel telecom của chúng em. Không biết {address} có thời gian chia sẻ cảm nhận về trải nghiệm sử dụng dịch vụ không ạ?\"\n"
    "2. Nếu khách đồng ý, hãy tiến hành khảo sát theo thứ tự sau, đảm bảo rằng mỗi câu hỏi chỉ được hỏi sau khi khách hàng đã trả lời câu hỏi trước:\n"
    "   a) Hỏi: \"{address} cảm thấy hài lòng nhất điều gì khi sử dụng gói dịch vụ {type}?\" (Ghi nhận: Điểm khách hàng hài lòng nhất)\n"
    "   b) Sau khi khách hàng trả lời, hỏi: \"Trong quá trình sử dụng gói dịch vụ {type}, có điểm nào {address} cảm thấy chưa hài lòng hoặc cần cải thiện không ạ?\" (Ghi nhận: Điểm cần cải thiện)\n"
    "   c) Tiếp theo, hỏi: \"{address} đánh giá thế nào về thái độ và sự hỗ trợ của nhân viên kỹ thuật, tổng đài hỗ trợ? (Xin {address} cho điểm từ 1 đến 5)\"\n"
    "   d) Cuối cùng, hỏi: \"Trong tương lai, {address} có mong muốn tiếp tục sử dụng dịch vụ của Viettel telecom hoặc có nhu cầu nâng cấp hoặc chuyển đổi sang gói cước khác không?\"\n"
    "3. Bắt buộc: cảm ơn khách hàng rằng: \"Cảm ơn {address} rất nhiều vì đã chia sẻ. Phản hồi của {address} sẽ giúp Viettel telecom nâng cao chất lượng dịch vụ. Em sẽ kết thúc cuộc gọi ngay bây giờ. Chúc {address} một ngày tốt lành!\" "
...
async def entrypoint(ctx: JobContext):
    ...
    instructions = (
        _default_instructions
        + f"Thông tin khách hàng: Tên: {customer_name}, Gói cước: {type}, Xưng hô: {address}.\n"
    ...
    run_voice_pipeline_agent(ctx, participant, instructions, phone_number, customer_name, address, type)
...
class CallActions(llm.FunctionContext):
    ...
    async def send_webhook(
        self,
        satisfaction_point: Annotated[str, "Điểm khách hàng hài lòng nhất về gói mạng sử dụng"],
        ...
        future_request: Annotated[str, "Nhu cầu sử dụng dịch vụ trong tương lai"]
    ):
        """
        ...
        - Điểm cần cải thiện
        - Đánh giá nhân viên kỹ thuật, tổng đài hỗ trợ (1-5 điểm)
        - Nhu cầu sử dụng trong tương lai
        """
        url = "https://hook.eu2.make.com/lvoybq4ns3hobab5c28znl6q6a6vocub"
        data = {
...
def run_voice_pipeline_agent(
    ctx: JobContext, participant: rtc.RemoteParticipant, instructions: str, phone_number: str, customer_name: str, address: str, type: str
):
    ...
    asyncio.create_task(
        agent.say(
            f"Chào {address} {customer_name}, em là Uyên nhân viên chăm sóc khách hàng từ Viettel telecom. Cảm ơn {address} đã sử dụng gói dịch vụ {type} của chúng em."
            f"Không biết {address} có thể dành chút thời gian để chia sẻ cảm nhận về trải nghiệm sử dụng dịch vụ trong thời gian vừa qua không ạ?",
